 ...
 * @author Aaron
 */
public class FourthMenu extends Menu {
    ...
    public FourthMenu(int width, int height, int xloc, int yloc) {
    ...
    private void addButtons() {
        // weapons button
        buttons.add(new ClickableButton(10, xloc, 25, yloc, "MAIN", 6));
    ...
    public void setPanes() {
        ...
        for (int i = 0; i < player.inventory.size(); i++) {
            if (player.inventory.get(i).getClass().getSimpleName().equals("WeaponItem")) {
                itemPane.add(new ItemPane(count, player.inventory.get(i), "EQUIP", 10));
                count++;
            }
        }
        if (count < 8) {
    ...
    public void render(Screen screen) {
        super.render(screen);
        font.renderSmallCharacters(27, 9, "Weapons", screen);
        for (int i = 0; i < buttons.size(); i++) {
